...
export default function Register() {
  ...
  // We keep all form data in a single state object, but validate per step
  const [formData, setFormData] = useState<any>({});

  const personalForm = useForm<PersonalDataValues>({
    resolver: zodResolver(personalDataSchema),
    defaultValues: { fullName: "", position: "" },
  });
  ...
  const bankForm = useForm<BankDataValues>({
    resolver: zodResolver(bankDataSchema),
    defaultValues: { bik: "", accountNumber: "", corrAccount: "", bankName: "" },
  });

  const logisticianForm = useForm<LogisticianValues>({
    resolver: zodResolver(logisticianSchema),
    defaultValues: { logistPhone: "", logistEmail: "" },
  });

  const handleStep1 = (data: PersonalDataValues) => {
    setFormData((prev: any) => ({ ...prev, ...data }));
    setStep(2);
  };

  const handleStep2 = (data: CompanyDataValues) => {
    setFormData((prev: any) => ({ ...prev, ...data }));
    setStep(3);
  };

  const handleStep3 = (data: BankDataValues) => {
    setFormData((prev: any) => ({ ...prev, ...data }));
    setStep(4);
  };
      ...
      <Card className="w-full max-w-lg">
        <CardHeader>
          <div className="flex items-center justify-between mb-2">
             ...
             >
               {step > 1 && <ArrowLeft className="h-4 w-4 mr-1" />}
               {step > 1 ? "Назад" : ""}
             </Button>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Менеджер по логистике" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <Form {...companyForm}>
              <form onSubmit={companyForm.handleSubmit(handleStep2)} className="space-y-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Название компании (с формой собственности)</FormLabel>
                      <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="г. Москва, ул. Пушкина, д. 10" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="10 или 12 цифр" maxLength={12} {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="13 или 15 цифр" maxLength={15} {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <Form {...bankForm}>
              <form onSubmit={bankForm.handleSubmit(handleStep3)} className="space-y-4">
                ...
                <div className="flex gap-2 mt-4">
                    <Button type="button" variant="outline" className="w-1/3" onClick={() => handleStep3(bankForm.getValues())}>
                        Пропустить
                    </Button>
          ...
          {step === 4 && (
            <Form {...logisticianForm}>
              <form onSubmit={logisticianForm.handleSubmit(handleFinalSubmit)} className="space-y-4">
                <FormField
                  control={logisticianForm.control}
                  name="logistPhone"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Телефон логиста</FormLabel>
                      <FormControl>
                        <Input placeholder="+7..." {...field} />
                      </FormControl>
                      <FormMessage />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Email логиста</FormLabel>
                      <FormControl>
                        <Input placeholder="logist@company.com" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
